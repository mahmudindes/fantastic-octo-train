...
#[Route('/api/rest/v0/comics/{code}/covers', name: 'rest_comic_cover_')]
class RestComicCoverController extends AbstractController
{
    ...
    ): Response
    {
        ...
        $result = $this->comicCoverRepository->findByCustom($criteria, $orderBys, $limit, $offset);
        $totalCount = $this->comicCoverRepository->countCustom($criteria);
    ...
    {
        $entity = new ComicCover();

        $form = $this->createForm(ComicCoverType::class, $entity, ['csrf_protection' => false]);
        ...
        if (!$form->isValid()) {
            return $this->json(['message' => 'Comic Cover data is not valid.'], Response::HTTP_BAD_REQUEST);
        }
    ...
    {
        $entity = $this->comicCoverRepository->findOneBy([
            'comic' => $this->comicRepository->findOneBy(['code' => $code]),
            'ulid' => $ulid
        ]);
        if (!$entity) {
            return $this->json(['message' => 'Comic Cover does not exists.'], Response::HTTP_NOT_FOUND);
        }
